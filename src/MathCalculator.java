...
public class MathCalculator {
	...
	private static final float PI_SLICE = (float)(Math.PI * 2 / (float) SCALE);
	private static final float sinTable[] = new float[SCALE];
	private static final float cosTable[] = new float[SCALE];
	...
	/*public static float sin(double radiant) {
		long lookUpIndex = (long) (radiant / PI_SLICE);
		if (lookUpIndex < 0)
			lookUpIndex *= -1;
		return sinTable[(int) (lookUpIndex % SCALE)];
	}

	public static float cos(double radiant) {
		long lookUpIndex = (long) (radiant / PI_SLICE);
		if (lookUpIndex < 0)
			lookUpIndex *= -1;
		return cosTable[(int) (lookUpIndex % SCALE)];
	}*/
	...
	public static float volumeCube(float centerX, float centerY, float centerZ,
	...
	public static float reduceTrig(float value) {
		float d = value / MathCalculator.TWOPI;
		return (d - ((int) d)) * MathCalculator.TWOPI;
	}
	...
	public static Color lerp(Color c0, Color c1, float amount) {
		...
		return new Color((int) lerp(c0.getRed(), c1.getRed(), amount),
				(int) lerp(c0.getGreen(), c1.getGreen(), amount), (int) lerp(
						c0.getBlue(), c1.getBlue(), amount), (int) lerp(
						c0.getAlpha(), c1.getAlpha(), amount));
	}
	...
	public static float volumeCone(float radius, float height) {
		return 0.333333333333333333333333333333333333333f * circleArea(radius)
				* height;
	}
	...
	public static float volumeIcosahedron(float radius) {
		...
		double sideLength = (Math
				.sqrt(0.0852211291184773171713198623624001074389829148814502
						* radius + 0.44721357712494718006225029347826 * radius));
