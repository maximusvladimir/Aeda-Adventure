...
public class Utility {
	...
	public static void drawMap(Graphics g, Main m, Player p, Gem[] gems,
			Tree[] trees, Sign[] signs,WaterFeature water, float walkX, float walkZ) {
		...
		Polygon formulator = new Polygon();
		for (int i = 0; i < water.getControlPoints().length; i++) {
			P3D controlPoint = water.getControlPoints()[i];
			formulator.addPoint(
					(int) ((controlPoint.x + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + mapx,
					(int) ((controlPoint.z + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + mapy);
		}
		g.fillPolygon(formulator);
		}
		...
		for (int i = 0; i < gems.length; i++) {
			if (!gems[i].isVisible())
				continue;
			int entityx = (int) ((gems[i].staticPos.x + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			int entityz = (int) ((gems[i].staticPos.z + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			// g.fillRect(mapx+entityx+5,mapy+entityz+5,3,3);
		...
		for (int i = 0; i < trees.length; i++) {
			int entityx = (int) ((trees[i].staticPos.x + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			int entityz = (int) ((trees[i].staticPos.z + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			g.setColor(new Color(144, 109, 74));
			g.fillRect(mapx + entityx + 5 - 1, mapy + entityz + 5 - 1, 3, 3);
			g.setColor(Color.black);
			// g.drawRect(mapx+entityx+5-1,mapy+entityz+5-1,3,3);
			g.setColor(new Color(0, 190, 0));
			g.drawLine(mapx + entityx + 5, mapy + entityz + 5, mapx + entityx
					+ 5, mapy + entityz + 5);
		}
		for (int i = 0; i < signs.length; i++) {
			int entityx = (int) ((signs[i].staticPos.x + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			int entityz = (int) ((signs[i].staticPos.z + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE);
			g.setColor(new Color(200,200,70));
			//g.fillRect(mapx + entityx + 5 - 1, mapy + entityz + 5 - 1, 3, 3);
			g.drawLine(mapx + entityx + 5 - 1, mapy + entityz + 5,
					mapx + entityx + 5 + 1, mapy + entityz + 5);
		}
		g.setColor(Color.white);
		g.fillRect((int) ((walkX + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + mapx + 4, mapy
				+ (int) ((walkZ + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + 4, 5,5);
		g.setColor(GameState.instance.playerColor);
		g.fillRect((int) ((walkX + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + mapx + 5, mapy
				+ (int) ((walkZ + GamePlane.WORLDSIZEHALF) * mapw / GamePlane.WORLDSIZE) + 5, 3, 3);
	}
