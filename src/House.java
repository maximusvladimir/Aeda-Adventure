...
public class House extends Drawable {
	...
	public void draw(int darkness) {
		...
		//delta += 0.05f;
		drawPane(new P3D(-890,600,395), new P3D(0,0,395),new P3D(890,600,395), new P3D(0,0,395));
		drawPane(new P3D(-895,600,-395), new P3D(0,0,-395),new P3D(895,600,-395), new P3D(0,0,-395));
		drawPaneSide(new P3D(-895,600,0), new P3D(-895,0,400),new P3D(-895,600,0), new P3D(-895,0,-400));
		drawPaneSide(new P3D(895,600,0), new P3D(895,0,400),new P3D(895,600,0), new P3D(895,0,-400));
		...
		drawLength(new P3D(715,0, 403), new P3D(760,600,403));
		drawLength(new P3D(560,0, 403), new P3D(520,600,403));
		drawLength(new P3D(322,0, 403), new P3D(367,600,403));	
		//drawLength(new P3D(900,500,403), new P3D(150,460,403));
		//drawLength(new P3D(900,100,403), new P3D(150,140,403));
		
		drawSideRoof(new P3D(-895,600,-400), new P3D(-895,600,400),200);
		drawSideRoof(new P3D(895,600,-400), new P3D(895,600,400),200);
	...
	public void tick() {
		...
		//float dy = pos.y;
		float dz = (getInstanceLoc().z - usz)+875; // make it slightly in front of the house.
	...
	private void drawRoof(P3D p0, P3D p1, float height) {
		...
		tesselator.point(0,p0.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p0.y,p0.z);
		tesselator.point(p0);
		
		tesselator.point(p1.x,p1.y,p0.z);
		tesselator.point(p1.x,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y,p0.z);
		tesselator.point(p1.x,p1.y,p0.z);
		...
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y,p1.z);
		tesselator.point(p1);
		
		tesselator.point(p0.x,p0.y,p1.z);
		tesselator.point(p0.x,p0.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y+height,(p0.z + p1.z)/2);
		tesselator.point(0,p1.y,p1.z);
	...
	private void drawSideRoof(P3D p0, P3D p1, float height) {
		...
		tesselator.point((p0.x+p1.x)/2,(p0.y+p1.y)/2 + height, (p0.z+p1.z)/2);
