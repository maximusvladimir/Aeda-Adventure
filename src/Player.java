...
public class Player extends Character {
	...
	public Player(Scene<Drawable> scene) {
		super(scene,
				new Hitbox(new P3D(-110, -225, -40), new P3D(110, 140, 40)));
		tesselator = new PointTesselator();
		tesselator.setDrawType(DrawType.Triangle);
		mouthExpression = FacialExpression.MOUTH_SMILE;
		eyeExpression = FacialExpression.EYEBROWS_INNOCENTLIKE;
		if (GameState.instance.playerGUID.equals("Zelda")
				|| GameState.instance.playerGUID.equals("Sheik")) {
			isASheik = true;
		} else
			isASheik = false;
	}
	...
	private void drawCurlThing(Color base, P3D startT0, P3D startT1,
			P3D startB0, P3D outward) {
		tesselator.color(va.variate(
				Utility.adjustBrightness(base, va.nextInt(20)), 15));
	...
	public void draw(int d) {
		...
		if (raftMode)
			tesselator.translate(pos.x, getScene().getGamePlane()
					.getSparsalHeight() + 100, pos.z - 200, false);
		else
		...
		// curl things:
		drawCurlThing(hairBase, new P3D(-40, 130, -20), new P3D(-40, 110, 20),
				new P3D(-40, 100, 0), new P3D(-60, 90 + wind, 0));
		drawCurlThing(hairBase, new P3D(-38, 130, -40), new P3D(-38, 140, -30),
				new P3D(-40, 110, -35), new P3D(-55, 160 + wind, -30));

		drawCurlThing(hairBase, new P3D(-10, 130, -60), new P3D(15, 120, -60),
				new P3D(0, 100, -60), new P3D(25, 140 + wind, -90));

		drawCurlThing(hairBase, new P3D(-35, 100, -30), new P3D(-35, 90, 10),
				new P3D(-35, 60, 0), new P3D(-70, 50, -10));
		drawCurlThing(hairBase, new P3D(-35, 70, 20), new P3D(-35, 90, 40),
				new P3D(-35, 50, 30), new P3D(-75, 30, 20));
		...
		/*
		 * tesselator.color(121, 49, 47); tesselator.point(-20, 48, -60);
		 * tesselator.point(0, 38 + mouth, -60); tesselator.point(20, 48, -60);
		 */
		...
		drawLeg(pants, bootsBase, adj1, alt1, feetHeight, feetTotal, feer,
				feera, feet, consanFeet, false);
		drawLeg(pants, bootsBase, adj2, alt2, feetHeight, feetTotal, feer2,
				feer2a, feet2, consanFeet, true);
		// drawLeg(Color pants, Color bootsBase, float adj1, float alt1, float
		// feetHeight, float feetTotal,float feer,
		...
		drawArm(foreArmColor, clothBase, skinBase, arm1, 0.0f + armsLag, true,
				d);
	...
	private void drawArm(Color bootsBase, Color clothBase, Color skinBase,
			float arm1, float amount909, boolean left, int dar) {
		...
		tesselator.point(bit * -75, -120 + armPosY2, -20 + armSwayMid
				+ armPosX2);
		...
		tesselator.point(bit * -100, -120 + armPosY2, -20 + armSwayMid
				+ armPosX2);
		...
		if (!left && GameState.instance.hasSword) {
			final Color swordBase = Utility.adjustBrightness(new Color(168,
					170, 187), -dar);
			tesselator.color(Utility.adjustBrightness(swordBase, 20));
			tesselator.point(-25 + bit * -120, -120 + (armPosY * 1.3f),
					(armSwayMid + armPosX) * 1.3f);

			tesselator.point(bit * -93,
					210 + ((armPosY + (armPosX * 3)) * 1.3f), -10
							+ (armSwayMid + (armPosX * 3)) * 1.3f);

			tesselator.point(-25 + bit * -65, -120 + (armPosY * 1.3f),
					(armSwayMid + armPosX) * 1.3f);

			tesselator.color(Utility.adjustBrightness(swordBase, -20));
			tesselator.point(-25 + bit * -110, -120 + (armPosY * 1.3f),
					(armSwayMid + armPosX) * 1.3f);

			tesselator.point(-35 + bit * -93, -150 + ((armPosY) * 1.3f),
					10 + (armSwayMid + (armPosX)) * 1.3f);

			tesselator.point(-25 + bit * -75, -120 + (armPosY * 1.3f),
					(armSwayMid + armPosX) * 1.3f);
		} else if (left && !GameState.instance.hasLantern) {
			final Color shieldBase = Utility.adjustBrightness(new Color(80, 90,
					80), -dar);
			tesselator.color(va.bright(shieldBase, 20));
			tesselator.point(bit * -130, -120 + (armPosY * 1.3f),
					35 + (armSwayMid + armPosX) * 1.3f);
			tesselator.point(bit * -93, -60 + (armPosY * 1.3f),
					5 + (armSwayMid + armPosX) * 1.3f);
			tesselator.point(bit * -55, -120 + (armPosY * 1.3f),
					35 + (armSwayMid + armPosX) * 1.3f);

			tesselator.color(va.bright(shieldBase, 20));
			tesselator.point(bit * -130, -120 + (armPosY * 1.3f),
					35 + (armSwayMid + armPosX) * 1.3f);
			tesselator.point(bit * -93, -160 + (armPosY * 1.3f),
					50 + (armSwayMid + armPosX) * 1.3f);
			tesselator.point(bit * -55, -120 + (armPosY * 1.3f),
					35 + (armSwayMid + armPosX) * 1.3f);
		}

		if (GameState.instance.hasFishOil && GameState.instance.hasLantern
				&& left)
			drawLantern(dar, arm1, armSwayMid, armPosX, armPosY, bit);
	}

	public Color lanternColor = Color.yellow;

	private void drawLantern(int darkness, float arm1, float armSwayMid,
			float armPosX, float armPosY, byte bit) {
		Color lanternBase = Utility.adjustBrightness(new Color(132, 115, 100),
				-darkness);
		tesselator.color(va.bright(lanternBase, 20));
		tesselator.point(bit * -108, -120 + (armPosY * 1.3f),
				40 + (armSwayMid + armPosX) * 1.3f);
		tesselator.point(bit * -115, -155 + (armPosY * 1.8f),
				35 + (-arm1 + armPosX) * 1f);
		tesselator.point(bit * -97, -120 + (armPosY * 1.3f),
				40 + (armSwayMid + armPosX) * 1.3f);

		tesselator.color(va.bright(lanternBase, 20));
		tesselator.point(bit * -72, -120 + (armPosY * 1.3f),
				40 + (armSwayMid + armPosX) * 1.3f);
		tesselator.point(bit * -60, -155 + ((armPosY) * 1.8f),
				35 + (-arm1 + armPosX) * 1f);
		tesselator.point(bit * -83, -120 + (armPosY * 1.3f),
				40 + (armSwayMid + armPosX) * 1.3f);

		tesselator.color(va.bright(lanternBase, 20));
		tesselator.point(bit * -108, -110 + (armPosY * 1.3f),
				15 + (armSwayMid + armPosX) * 1.3f);
		tesselator.point(bit * -115, -155 + (armPosY * 1.8f),
				16 + (-arm1 + armPosX) * 1f);
		tesselator.point(bit * -97, -110 + (armPosY * 1.3f),
				15 + (armSwayMid + armPosX) * 1.3f);

		tesselator.color(va.bright(lanternBase, 20));
		tesselator.point(bit * -72, -110 + (armPosY * 1.3f),
				15 + (armSwayMid + armPosX) * 1.3f);
		tesselator.point(bit * -60, -155 + ((armPosY) * 1.8f),
				16 + (-arm1 + armPosX) * 1f);
		tesselator.point(bit * -83, -110 + (armPosY * 1.3f),
				15 + (armSwayMid + armPosX) * 1.3f);

		tesselator.color(va.variate(lanternBase, 20));
		tesselator.point(bit * -60, -155 + ((armPosY) * 1.8f),
				16 + (-arm1 + armPosX) * 1f);
		tesselator.point(bit * -60, -155 + ((armPosY) * 1.8f),
				35 + (-arm1 + armPosX) * 1f);
		...
		if (flameSize > 0.2f) {
			float waveXx = (float) (MathCalculator.cos(lanternWave) * 20);
			float waveZz = (float) (MathCalculator.sin(lanternWave * 1.1f) * 20);
			float waveSs = (float) (MathCalculator.sin(lanternWave) * Math
					.tan(lanternWave * 0.75f));
			if (waveSs > 1)
				waveSs = 1;
			if (waveSs < 0.4f)
				waveSs = 0.4f;
			waveSs = waveSs * 10;
			tesselator.color(lanternColor);
			tesselator.point(bit * -80 + waveSs, -155 + ((armPosY) * 1.8f),
					16 + (-arm1 + armPosX) * 1f);
			tesselator.point(bit * -95 + waveXx, (-160 + (flameSize * 14))
					+ ((armPosY) * 1.8f), 23 + (-arm1 + armPosX) * 1f + waveZz);
			tesselator.point(bit * -95 - waveSs, -155 + ((armPosY) * 1.8f),
					35 + (-arm1 + armPosX) * 1f);
		}
	}
	...
	private float lanternWave;
	private float lanternDelta;
	
	private int iniddDark = 0;
	public void setPlayerIndividualDarkness(int v) {
		iniddDark = v;
	}
	
	public int getPlayerIndividualDarkness() {
		return iniddDark;
	}

	private void drawLeg(Color pants, Color bootsBase, float adj1, float alt1,
			float feetHeight, float feetTotal, float feer, float feera,
			float feet, float consanFeet, boolean right) {
		...
		if (feet < 0)
			tesselator.color(Utility.adjustBrightness(pants,
					va.nextInt(-30, -20)));
		else
			tesselator.color(Utility.adjustBrightness(pants,
					va.nextInt(-20, -10)));
		...
		tesselator.point(-75 * bit + adj1c, feetHeight + feerac + 5, -35
				+ feetc);
		tesselator.point(-15 * bit + adj1c, feetHeight + feerac + 5, -35
				+ feetc);
		tesselator
				.point(-30 * bit + adj1, feetTotal + feera, -45 + feet + alt1);
		tesselator.color(Utility.adjustBrightness(bootsBase,
				va.nextInt(-20, -10)));
		tesselator
				.point(-30 * bit + adj1, feetTotal + feera, -45 + feet + alt1);
		tesselator
				.point(-60 * bit + adj1, feetTotal + feera, -45 + feet + alt1);
		tesselator.point(-75 * bit + adj1c, feetHeight + feerac + 5, -35
				+ feetc);
		...
		tesselator
				.point(-60 * bit + adj1, feetTotal + feera, -45 + feet + alt1);
		tesselator.point(-55 * bit + adj1, feetTotal + (feera * 1.5f), -90
				+ feet + alt1);
		tesselator.color(Utility.adjustBrightness(bootsBase,
				va.nextInt(-10, -15)));
		tesselator
				.point(-30 * bit + adj1, feetTotal + feera, -45 + feet + alt1);
		tesselator.point(-35 * bit + adj1, feetTotal + (feera * 1.5f), -90
				+ feet + alt1);
		tesselator.point(-55 * bit + adj1, feetTotal + (feera * 1.5f), -90
				+ feet + alt1);
		...
		tesselator.color(Utility.adjustBrightness(bootsBase,
				va.nextInt(-20, -10)));
		tesselator
				.point(-75 * bit + adj1c, feetHeight + feerac + 5, 30 + feetc);
		tesselator
				.point(-15 * bit + adj1c, feetHeight + feerac + 5, 30 + feetc);
	...
	public void tick() {
		...
		if (flameSize > 0.001f) {
			flameSize -= 0.001f;
			setPlayerIndividualDarkness((int)(-flameSize * 30)-50);
			float hue = 1-(float)(Math.abs((MathCalculator.cos(lanternDelta)*0.1666666666666)));
			lanternColor = new Color(Color.HSBtoRGB(1-hue, 1, 1));
		}
		else
			setPlayerIndividualDarkness(0);

		if (getScene() != null
				&& (getScene().getLevel().isMessageBeingShown() || getScene()
						.getLevel().isInConsoleMode()))
			moving = false;
		...
		if (movingTowards) {
			...
			getScene().setPlayerPosition(
					new P3D(startX - ux * destAlt, 0, startZ - uz * destAlt));
			if (destAlt >= dist) {
				movingTowards = false;
				destX = 0;
				destZ = 0;
	...
	public void moveTowards(P3D p) {
		...
		dist = p.dist(new P3D(getScene().getPlayerX(), 0, getScene()
				.getPlayerZ()));
		destAlt = 0;
		movingTowards = true;
	}
